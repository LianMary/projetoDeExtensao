# main.py
#adição de email e nome no token para validação no submit_results
...
def login_user(data: LoginData): 
    ...
    print(f"--> Requisição de LOGIN recebida. Nome: {data.nome}, Telefone: {data.telefone}, email {data.email}") #
    ...
    aluno_db_match = sheets_service.STUDENTS_DATABASE[(sheets_service.STUDENTS_DATABASE['TELEFONE'].astype(str) == telefone_busca)] 
    
    
    if aluno_db_match.empty:
        email_check = sheets_service.STUDENTS_DATABASE[
        sheets_service.STUDENTS_DATABASE["EMAIL"].str.strip() .str.lower() == email_busca_lower
        ]
    ...
    # 6. ATUALIZAÇÃO DE E-MAIL (Se o campo estiver vazio na planilha, aceite o novo e-mail)
    if not email_correto_na_planilha:
        print("INFO: Adicionando E-mail ao registro existente (campo estava vazio).")
        # NOTA: O e-mail será persistido no próximo /submit_results
...
def submit_results(data: ResultadoQuestionario, token: str = Depends(oauth2_scheme)):
    ...
    if email_do_token !=data.email.strip():
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
